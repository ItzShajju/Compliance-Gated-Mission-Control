...
st.set_page_config(layout="wide", page_title="Compliance-Gated Mission Control", page_icon="??")
 
# Embedded extended sample CSV (realistic fields)
...
def sw_before_after(df, cutoff_days=15):
    ...
    rows=[]
    for v in versions:
        rows.append({'sw_version':v,'before':int(before.get(v,0)),'after':int(after.get(v,0)),'delta':int(after.get(v,0))-int(before.get(v,0))})
...
# --- Evidence bundle creation (PDF+CSV+JSON zip) ---
def create_evidence_zip(selected_df, title, user, priority, charts=[]):
    ...
    # Add first N rows as table text
    c.setFont("Helvetica-Bold", 12)
...
# --- UI styling (mission-control look) ---
st.markdown("""
<style>
...
with left:
    ...
    fig_sw = px.bar(sw_df.melt(id_vars='sw_version', value_vars=['before','after']), x='sw_version', y='value', color='variable', barmode='group', title=f"SW version before vs after (cutoff {cutoff.date()})")
    ...
    v21_series.columns = ['date','aids']; v21_series['date'] = pd.to_datetime(v21_series['date'])
    ...
    if hum:
        a,b,delta = hum
        insight += f" Average humidity for v2.1 events = {a:.0f}% vs others {b:.0f}% (?={delta:.0f}%)."
    st.info(insight)
    ...
    st.markdown("---")
    st.markdown("### Drilldown / Data Explorer")
    cols_f = st.columns(4)
    tails = ["All"] + sorted(df['tail'].unique().tolist())
    tail_sel = cols_f[0].selectbox("Tail", tails)
    sw_sel = cols_f[1].selectbox("SW Version", ["All"] + sorted(df['sw_version'].unique().tolist()))
    ata_sel = cols_f[2].selectbox("ATA Chapter", ["All"] + sorted(df['ata_chapter'].astype(str).unique().tolist()))
